...
import { SearchParams, ApartmentListing } from '../types';
import dotenv from 'dotenv';
import { findCompanyOffice } from './companyLocationService';
...
const mockApartments: ApartmentListing[] = [
  {
    id: '1',
...
function mapFeaturesToAmenities(features: any): string[] {
  if (!features) return [];
  const amenities: string[] = [];
  if (features.cooling) amenities.push('Air Conditioning');
  if (features.pool) amenities.push('Pool');
  if (features.garage) amenities.push('Garage');
  if (features.fireplace) amenities.push('Fireplace');
  if (features.heating) amenities.push('Heating');
  if (features.laundry) amenities.push('Washer/Dryer');
  if (features.furnished) amenities.push('Furnished');
  // Add more mappings as needed
  return amenities;
}

async function fetchPropertyFeatures(address: string): Promise<any> {
  try {
...
async function searchRentCast(params: SearchParams): Promise<ApartmentListing[]> {
  if (!RENTCAST_API_KEY) {
  ...
  try {
    ...
    const officeLocation = await findCompanyOffice(params.company, params.location);
    ...
    const [city, state] = params.location.split(',').map((part: string) => part.trim());
    ...
    const filteredListings = listings.filter(listing => 
      listing.price <= params.maxPrice
    );

    // Process listings without making additional API calls
    const processedListings = filteredListings.map((listing: any) => {
      ...
      return {
        ...
        distance,
        rating: 4.0,
        image: listing.photoUrls?.[0] || listing.photos?.[0] || 'https://images.unsplash.com/photo-1522708323590-d24dbb6b0267',
        amenities,
...
async function searchRent(params: SearchParams): Promise<ApartmentListing[]> {
  try {
    const response = await axios.get('https://api.rent.com/api/v1/listings', {
      params: {
...
async function searchHotPads(params: SearchParams): Promise<ApartmentListing[]> {
  try {
    const response = await axios.get('https://api.hotpads.com/v1/listings', {
      params: {
        location: params.location,
        maxPrice: params.maxPrice,
        bedrooms: params.roomType === 'studio' ? 0 : parseInt(params.roomType),
...
export async function searchApartments(params: SearchParams, recommendedIds: string[] = []): Promise<ApartmentListing[]> {
  try {
    // Try to get real data from RentCast first
    const rentcastResults = await searchRentCast(params);
